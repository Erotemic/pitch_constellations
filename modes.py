...
chromatic_xy = np.array(list(zip(*cirlce_rad2xy(chromatic_theta))))
...
def build_modes(base_scale, name_list):
    if not isinstance(name_list, list):
        name_list = [str(name_list) + str(num)
                     for num in range(len(base_scale))]
    base_shift = ring_diff(base_scale)
    modes = {}
    for ix in range(len(base_scale)):
...
def hex_to_rgb(value):
    value = value.lstrip('#')
    lv = len(value)
    return tuple(float(int(value[i:i + int(lv / 3)], 16)
                       ) / 255 for i in range(0, lv, int(lv / 3)))
...
def plot_constellation(scale, pos_xy=(0., 0.), scale_name=None, **kwargs):
    """
    Example:
        >>> scale = np.array([ 0,  1,  3,  5,  6,  8, 10])
        >>> pos_xy = (-12.96, 0.0)
        >>> scale_name = 'Locrian'
    """
    # fig = plt.gcf()
    ...
    # Plot background
    for note_x, (chrm_xy, chrm_theta) in enumerate(zip(chromatic_xy, chromatic_theta)):
        arm_x, arm_y = chrm_xy
    ...
    for scale_x, (arm_xy, arm_theta) in enumerate(zip(scale_xy, scale_theta)):
        ...
        if scale_x == 0:
            ...
            hand_artist = plt.Circle(
                (pos_x + arm_x, pos_y + arm_y), .05, color=arm_color)
